...
mm = mm %>% group_by(correlation,network) %>% 
  mutate(by_all = { 
    min.conn = min(map_dbl(allowed.conn,max))  #1.123148 
...
saveRDS(mm, "soft-thresholding-pwrs_fulltable_noquantile_Neuclean_GEclean_newcuts_noPCA(dlpfc new groups).rds")