...
minAge = -1;maxAge = 150; minRin = 6; diagnosis = c("Schizo"); race = c("AA","CAUC")
...
remove_outlier_samples = function(rse, name.rse , sdout = 3){
  ...
  plot(scaled.IAC[match(labels(dend),names(scaled.IAC))], ylim = c(-8,10), pch = 19, col = cll, ylab = "z-scored sample")
...
rse_outlier_removed = rse_preprocess_step1 %>% purrr::imap( ~{
...
##########Additional outlier removed manually
rse_outlier_removed_age_grps$dlpfc.O = rse_outlier_removed_age_grps$dlpfc.O[,!colnames(rse_outlier_removed_age_grps$dlpfc.O) %in% "R3496"]
rse_outlier_removed_age_grps$hippo   = rse_outlier_removed_age_grps$hippo[,!colnames(rse_outlier_removed_age_grps$hippo) %in% c("R5661","R4857")]
...
rse_outlier_removed_big = list(
  ...
  dentate = do.call(cbind,rse_outlier_removed_age_grps[grep("dentate", names(rse_outlier_removed_age_grps))])
)

colData(rse_outlier_removed_big$dentate)$Age_grp = "gr__-1_100"
rse_outlier_removed_age_grps = NULL
#rse_outlier_removed_age_grps$dentate = NULL
############
...
rse_preprocess_step2 = imap(rse_outlier_removed_big, ~{
  ...
  assays(rse_merged2)$counts    = NULL
  return(rse_merged2)
})
...
get.cells.proportions = function(rse,rse.name, assay.name, method, nmarkers){
  ...
  zz = brainCells(exp, nMarker=nmarkers, species="human", method = method)
  return(zz)
}
...
setwd("results/")
saveRDS(rse_preprocess_step3      , "rse_preprocessed_SchizoNew_GEclean_newcuts.rds")         #List of rse object preprocessed (sample and gene outlier removed. Quantile)
