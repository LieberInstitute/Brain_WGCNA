...
rse_sva_cleaned_sims      = readRDS("rse_sva_cleaned_protectDx_KOsims(redone).rds")


pwr.df = readRDS("soft-thresholding-pwrs_fulltable_protectDx_KOsims(redone).rds")
pwr.df$full.name = gsub("_protectDx_noConsensus\\(redone\\)","",pwr.df$full.name)

n.samples = unlist(map(rse_sva_cleaned_sims,~dim(.)[2])) #Number of samples per set
n.genes   = unlist(map(rse_sva_cleaned_sims,~dim(.)[1])) #Number of genes   per set
...
setwd("C:\\Users\\mpariha1\\Desktop\\upload_to_google_drive\\OneDrive - Johns Hopkins\\Stem_Cell_Project_2021\\redone\\results\\KO sft estimation shuffeled\\KO nets\\")

assay.name = "ranknorm"
pwr.type   = "by_individual"
mod        = "pearson"

plan(list(tweak(multisession, workers = 2), tweak(multisession, workers = 3)))
net.pearson.shybrid   =  future_imap(rse_sva_cleaned_sims,~ get_networks(.x, nname = .y, mod = mod , assay.name = assay.name, beta.pwr = pwr.df, pwr.type = pwr.type, net.type = "signed hybrid"))
plan(sequential)
...
saveRDS(gene_module_list,"gene-module list(stem cells)_protectDx_KOsims(redone).rds")
...
metadf$match.used = gsub("pwr.type|_protectDx_noConsensus\\(redone\\)","",metadf$match.used)
#metadf$full.name = paste0(metadf$full.name, "_protectDx")
...
saveRDS(gene_module_metadf, "metadf_networks_protectDx_KOsims(redone).rds")
